class IncidentesController < ApplicationController
  ...
  def index
		...
		if @ptipo == "assalto"
			filtro = filtro + " and tipo = 'assalto' "
		end
		if @ptipo == "acidente"
		 	filtro = filtro + " and tipo = 'acidente' "
		end
		@incidentes = Incidente.where(filtro).order("tipo");
	end
  ...
  private
    ...
    def incidente_params
      params.require(:incidente).permit(:lat, :long, :tipo, :data, :hora, :descricao, :foto)
    end
